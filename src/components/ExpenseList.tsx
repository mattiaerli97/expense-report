...
export default function ExpenseList({ expenses, settlements, onDelete }: Props) {
  ...
  const filtered = rows.filter((r) => {
    ...
    if (tab === "settlements") return r.type === "settlement";
    return true;
  });
  ...
  const tabs: { id: TabType; label: string; count: number }[] = [
    { id: "all", label: "Tutto", count: rows.length },
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((row) => {
                if (row.type === "expense") {
                  const e = row.data;
                  const personLabel = PEOPLE.find((p) => p.id === e.paidBy)?.label ?? e.paidBy;
                  return (
                  ...
                  const s = row.data;
                  const fromLabel = PEOPLE.find((p) => p.id === s.from)?.label ?? s.from;
                  const toLabel = PEOPLE.find((p) => p.id === s.to)?.label ?? s.to;
                  return (
